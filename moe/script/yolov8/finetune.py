...
def finetune_simple_trainer_run_step(self):
    ...
    loss_dict_items =   {"Box loss": float(loss_components[0]),
                        "Class loss": float(loss_components[1]),
                        "DFL loss": float(loss_components[2])}
    loss_dict =   {"Box loss": loss_components[0],
...
class FinetuneTrainer(DefaultTrainer):
    def __init__(self, cfg, gmm_models=None):
        ...
        model = load_yolov8(os.path.join(os.path.dirname(__file__), cfg.YOLO_CONFIG_PATH), cfg.CKPT)
