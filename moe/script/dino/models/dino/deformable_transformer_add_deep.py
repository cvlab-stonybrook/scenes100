...
class DeformableTransformer(nn.Module):
    ...
    def forward(self, srcs, masks, refpoint_embed, pos_embeds, tgt, attn_mask=None, prompt=None):
        ...
        ### TEST VPT
        num_prompt_each_level = None
        prompt_start_index = None
        if prompt is not None:
            total_prompt = prompt.shape[0]
            num_prompt = int(prompt.shape[0] / self.num_encoder_layers)
            ...
            num_prompt_each_level = max_prompt_each_level
            
            # breakpoint()
            prompt = prompt.unsqueeze(0).expand(src_flatten.shape[0], total_prompt, src_flatten.shape[2])

            # prompt_index = 0
            # for i, num in enumerate(num_prompt_each_level):             
            #     src_flatten[:, prompt_start_index[i]: prompt_start_index[i]+num, :] = src_flatten[:, prompt_start_index[i]: prompt_start_index[i]+num, :] + prompt[:, prompt_index:prompt_index+num,:]
            #     prompt_index += num
        ###
        # two stage
        ...
        memory, enc_intermediate_output, enc_intermediate_refpoints = self.encoder(
                ...
                prompt = prompt,
                num_prompt_each_level = num_prompt_each_level,
...
class TransformerEncoder(nn.Module):
    ...
    def forward(self, 
        ...
        """
        if prompt is not None:
            num_prompt_each_layer = int(prompt.shape[1] / len(self.layers))
        ...
        for layer_id, layer in enumerate(self.layers):
            if prompt is not None:
                prompt_index = 0
                for i, num in enumerate(num_prompt_each_level):             
                    output[:, prompt_start_index[i]: prompt_start_index[i]+num, :] = output[:, prompt_start_index[i]: prompt_start_index[i]+num, :] + prompt[:, layer_id*num_prompt_each_layer+prompt_index:layer_id*num_prompt_each_layer+prompt_index+num,:]
                    prompt_index += num           
