...
class DeformableTransformer(nn.Module):
    ...
    def get_valid_ratio(self, mask):
        ...
        return valid_ratio, torch.stack([valid_W, valid_H], -1)
    ...
    def forward(self, srcs, masks, refpoint_embed, pos_embeds, tgt, attn_mask=None, prompt=None):
        """
        Input:
        ...
        valid_shapes = torch.stack([self.get_valid_ratio(m)[1] for m in masks], 1) # bs, num_lvl, 2
        # breakpoint()
        ### TEST VPT
        if prompt is not None:
            def calculate_new_shapes(old_shapes, ratio, max_prompt_each_level):
                from math import sqrt
                new_shapes = []
                num_prompt_each_level = []
                for i , (h, w) in enumerate(old_shapes):
                    new_h = int(h*sqrt(1+ratio))
                    new_w = int(w*sqrt(1+ratio))
                    new_shapes.append((new_h, new_w))
                    assert new_h * new_w - h * w <= max_prompt_each_level[i], f"not enough prompt for level {i}"
                    num_prompt_each_level.append(new_h * new_w - h * w)
                return new_shapes, num_prompt_each_level
            num_prompt = prompt.shape[0]
            num_tokens = [h*w for (h,w) in spatial_shapes]
            ratio = num_prompt/src_flatten.shape[1]

            prompt_start_index = level_start_index.clone().detach().tolist()
            prompt_start_index.append(src_flatten.shape[1])

            max_prompt_each_level = [int(ratio * num_token) for num_token in num_tokens]
            max_prompt_each_level[0] = num_prompt - sum(max_prompt_each_level[1:])
            
            prompted_spatial_shapes, num_prompt_each_level = calculate_new_shapes(spatial_shapes, ratio, max_prompt_each_level)
            breakpoint()
            # prompt = torch.rand(num_prompt, src_flatten.shape[2]).cuda()
            prompt = prompt.unsqueeze(0).expand(src_flatten.shape[0], num_prompt, src_flatten.shape[2])
            pos_embed_prompt = torch.zeros_like(prompt)
            mask_prompt = torch.zeros((src_flatten.shape[0], num_prompt), dtype=torch.bool).cuda()

            prompt_index = 0
            prompted_embeddings = []
            prompted_lvl_pos_embeddings = []
            prompted_masks = []
            prompted_lvl_start_index = []
            for i, num in enumerate(num_prompt_each_level):
                prompted_lvl_start_index.append(prompt_index)
                prompted_embedding = torch.cat([src_flatten[:, prompt_start_index[i]:prompt_start_index[i+1], :], prompt[:, prompt_index:prompt_index+num,:]], dim=1)
                prompted_lvl_pos_embedding = torch.cat([lvl_pos_embed_flatten[:, prompt_start_index[i]:prompt_start_index[i+1], :], pos_embed_prompt[:, prompt_index:prompt_index+num,:]], dim=1)
                prompted_mask = torch.cat([mask_flatten[:, prompt_start_index[i]:prompt_start_index[i+1]], mask_prompt[:, prompt_index:prompt_index+num]], dim=1)
                prompt_index += num
                prompted_embeddings.append(prompted_embedding)
                prompted_lvl_pos_embeddings.append(prompted_lvl_pos_embedding)
                prompted_masks.append(prompted_mask)
            prompted_embeddings = torch.cat(prompted_embeddings, dim=1) # bs, \sum{hxw} + num_prompt, c
            prompted_lvl_pos_embeddings = torch.cat(prompted_lvl_pos_embeddings, dim=1) # bs, \sum{hxw} + num_prompt, c
            prompted_masks =  torch.cat(prompted_masks, dim=1) # bs, \sum{hxw} + num_prompt
            
            src_flatten = prompted_embeddings
            lvl_pos_embed_flatten = prompted_lvl_pos_embeddings
            mask_flatten = prompted_masks
            level_start_index = torch.as_tensor(prompted_lvl_start_index, device=src_flatten.device)
            spatial_shapes = torch.as_tensor(prompted_spatial_shapes, dtype=torch.long, device=src_flatten.device)
            valid_ratios = valid_shapes / spatial_shapes[:, [1,0]]
            # breakpoint()
